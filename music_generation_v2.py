...
def autoencoder(X):
	...
	net = TimeDistributed(Dense(2000, activation='relu'))(net_in)
	import pdb; pdb.set_trace()
	net = TimeDistributed(Dense(200, activation='relu'))(net)
	net = Flatten()(net)
	...
	net = TimeDistributed(Dense(MAX_LENGTH * 200))(net)
	net = Reshape((MAX_LENGTH, 200))(net) #193
	net = BatchNormalization(momentum=BN_M)(net)
	net = Activation('relu')(net)
	net = Dropout(DROPOUT)(net) # 197

	net = TimeDistributed(Dense(2000))(net)
	net = BatchNormalization(momentum=BN_M)(net)
	net = Activation('relu')(net)
	net = Dropout(DROPOUT)(net) #204

	net = TimeDistributed(Dense(X.shape[2] * X.shape[3], activation='sigmoid'))(net)
...
rate, data = get_audio('/home/ksooklall/Music/wav/taki_taki.wav') 
import pdb; pdb.set_trace()
